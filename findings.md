...
## Research Findings
- backend/Dockerfile 与 Dockerfile.backend.gitee 的默认基础镜像已切换为华为云 SWR 同步的 Docker Hub `maven:3-eclipse-temurin-17` 与 `eclipse-temurin:17-jre-alpine`。
- frontend/Dockerfile 与 Dockerfile.frontend.gitee 的默认基础镜像已切换为华为云 SWR 同步的 Docker Hub `node:20-alpine` 与 `nginx:1.25-alpine`。
...
- 最新 Gitee Go 构建日志已确认 `registry.cn-hangzhou.aliyuncs.com/acs/openjdk:17-jre-alpine` 会返回 `insufficient_scope`，`registry.cn-hangzhou.aliyuncs.com/acs/node:20-alpine` 会返回 `not found`，因此当前阿里云默认 tag 不能继续使用。
- 公开镜像索引页可确认以下国内可拉取镜像地址：`swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/library/maven:3-eclipse-temurin-17`、`swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/library/eclipse-temurin:17-jre-alpine`、`swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/library/node:20-alpine`。

## Technical Decisions
| Decision | Rationale |
|----------|-----------|
| 服务器自动部署脚本继续基于 docker compose | 项目本身就是多容器，直接 docker run 需要手动管理网络、卷和健康依赖，复杂度高 |
| 基础镜像默认值改为华为云 SWR 同步的 Docker Hub 镜像，并保留 build-arg 覆盖能力 | 最新 Gitee Go 日志已证明当前阿里云默认 tag 不可用，改成已确认存在的国内镜像映射更稳，同时仍保留覆盖能力 |
...
| Issue | Resolution |
|-------|------------|
| Gitee Go 运行时无法拉取当前阿里云默认基础镜像 tag | 根据最新日志切换到已确认存在的华为云 SWR Docker Hub 镜像映射，并保留 build-arg 覆盖能力 |
...
- https://docker.aityp.com/image/docker.io/library/maven:3-eclipse-temurin-17
- https://docker.aityp.com/image/docker.io/library/eclipse-temurin:17-jre-alpine
- https://docker.aityp.com/image/docker.io/library/node:20-alpine

## Visual/Browser Findings
- `docker.aityp.com` 的镜像索引页给出了华为云 SWR 的 Docker Hub 镜像映射，确认了 `maven:3-eclipse-temurin-17`、`eclipse-temurin:17-jre-alpine`、`node:20-alpine` 的国内可拉取地址。
